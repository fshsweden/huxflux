...
class TargetsControllerTest < ActionDispatch::IntegrationTest
  ...
  test "should create target" do
    assert_difference('Target.count') do
      post targets_url, params: { target: { email: @target.email, list_id: @target.list_id, name: @target.name } }
    end
  ...
  test "should update target" do
    patch target_url(@target), params: { target: { email: @target.email, list_id: @target.list_id, name: @target.name } }
